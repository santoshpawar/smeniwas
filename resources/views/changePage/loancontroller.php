...
class LoansController extends BaseLoansController
{
  ...
  if (isset($loanId)) {
    ...
    if (($input['com_business_type'] == 'Manufacturing') || ($input['com_business_type'] == 'Trading')) {
    $fieldsArr['com_vat'] = $input['com_vat'];
    $rulesArr['com_vat'] = 'required';
    $messagesArr['com_vat.required'] = 'Vat No is required in KYC Details Tab.';
  }
}*/
...
if ($validator->fails()) {
  ...
  //'promoter_details' => 'N', 'financials' => 'N', 'business_details' => 'N', 'security_details' => 'N', 'upload_documents' => 'N', 'input_blsheet' => 'N', 'input_p&l' => 'N', 'calculated_ratios' => 'N', 'credit_model' => 'N', 'collateral_model' => 'N'
  $loansStatus->save();
  ...
  ->update(array('turnover' =>$userProfile->latest_turnover));*/
  //dd($input['sales_area_type']);

  /*  $salesAreaDetails = SalesAreaDetails::updateOrCreate(['loan_id' => $loan->id], ['loan_id' => $loanId, 'sales_area_type' => $input['sales_area_type'], 'city_name' => $input['city_name'],
      'city_name_1' => $input['city_name_1'], 'city_name_2' => $input['city_name_2'], 'multi_state_1' => $input['multi_state_1'],
      'multi_state_2' => $input['multi_state_2'], 'multi_state_3' => $input['multi_state_3'], 'multi_state_4' => $input['multi_state_4'],
      'multi_state_5' => $input['multi_state_5']]);
    $salesAreaDetails->save();
  }*/
  $user = Auth::user();
  if (isset($user)) {
    $userID = $user->id;
...
//public function getUploaddoc($loanType, $endUseList = null, $amount = null, $loanTenure= null, $loanId = null) {
  ...
  /*   echo $loanId."_loanid<br>";
  echo $endUseList."_enuse<br>";
  echo $loanType."_loantype<br>";
  echo $loanProduct."_loanproduct<br>";
  echo $amount."_amt<br>";
  echo $loanTenure."_tenure<br>";
  echo $companySharePledged."_companyName<br>";
  echo $bscNscCode."_Companycode<br>";
  echo $afterShare."_aftershare<br>";*/
  $loan = null;
  if (isset($loanId)) {
    $loan = Loan::find($loanId);
    //$loan = Loan::find($loanId);
...
public function postUploaddoc(Request $request)
{
  ...
  //$bl_year = MasterData::BalanceSheet_FY();
    ...
    //Final Year copy
      ...
      if($countYear<=1 && $input['fin_year_'.$countYear] == 0) {
      $fieldsArr['finyear_file' . $countYear . '_path'] = $request->file('finyear_file' . $countYear . '_path');
      $rulesArr['finyear_file' . $countYear . '_path'] = 'required';
      $messagesArr['finyear_file' . $countYear . '_path.required'] = 'Financials Reports/Balance Sheets' . $countYear . ' is required.';
    }
  }
}*/
}
...
if (isset($input['kycDetails'])) {
  for ($i = 1; $i <= count($input['kycDetails']); $i++) {
    ...
    foreach ($file_temp as $key => $item) {
      ...
      if($key == 'pancard_file_path' && $input['pan_card_check1'] == 0) {
      $fieldsArr['pancard_file_path'] = $item;
      $rulesArr['pancard_file_path'] = 'required';
      $messagesArr['pancard_file_path.required'] = 'Pan Card File is required.';
      //$messagesArr[$key . 'required'] = 'Pan Card File is required.';
    }elseif($key == 'addproof_file_path' && $input['addressproof_company_file_check'] == 0){
    $fieldsArr['addproof_file_path'] = $item;
    $rulesArr['addproof_file_path'] = 'required';
    $messagesArr['addproof_file_path.required'] = 'Address Proof of Company Business File is required.';
  }elseif($key == 'kyc_extra_file1_path' && $input['optional_file1_check']==0){
  $fieldsArr['kyc_extra_file1_path'] = $item;
  $rulesArr['kyc_extra_file1_path'] = 'required';
  $messagesArr['kyc_extra_file1_path.required'] = 'KYC Details Extra field File is required.';
}
}
}*/
}
...
$fieldsArr['prom_bank_stmt_file_path'] = $request->file('prom_bank_stmt_file_path');
$rulesArr['prom_bank_stmt_file_path'] = 'required';
$messagesArr['prom_bank_stmt_file_path.required'] = 'Promoter Bank Statements File is required.';
}
}
}*/
if ($request->file('prom_networth_file_path')) {
  ...
  if ($deletedQuestionHelper->isQuestionValid("F2.2")) {
    //                    if($input['networth_file_check']==0) {
    //                        $fieldsArr['prom_networth_file_path'] = $request->file('prom_networth_file_path');
    //                        $rulesArr['prom_networth_file_path'] = 'required';
    //                        $messagesArr['prom_networth_file_path.required'] = 'Networth Certificate File is required.';
    //                    }
  }
}
...
if($input['prom_kyc_addproof_file_path_check']==0) {
$fieldsArr['prom_kyc_addproof_file_path'] = $request->file('prom_kyc_addproof_file_path');
$rulesArr['prom_kyc_addproof_file_path'] = 'required';
$messagesArr['prom_kyc_addproof_file_path.required'] = 'Promoter Address Proof File is required.';
}
}
}*/
//            if ($request->file('prom_visiting_file_path')) {
//                $file = $request->file('prom_visiting_file_path');
//                $originalFileName = $file->getClientOriginalName();
//                $uploadedFileName = $directory. '/'. 'loan_' . $loanId . '-'. 'promoter_visitingcard' . '_' . $originalFileName;
//                $oldFileName = null;
//                $fieldName = 'prom_visiting_file_path';
//
//                if(!isset($uploadDetails)){
//                    $uploadDetails = Upload::updateOrCreate(array('loan_id'=>$loanId),array($fieldName => $uploadedFileName));
//                }
//                if(isset($uploadDetails) && $uploadDetails->__isset($fieldName)){
//                    $oldFileName = $uploadDetails->getAttribute($fieldName);
//                }
...
//            }else{
//                if($deletedQuestionHelper->isQuestionValid("F2.3")) {
//                    if($input['visitingcard_file_check'] == 0) {
//                        $fieldsArr['prom_visiting_file_path'] = $request->file('prom_visiting_file_path');
//                        $rulesArr['prom_visiting_file_path'] = 'required';
//                        $messagesArr['prom_visiting_file_path.required'] = 'Promoter Visiting Card File is required.';
//                    }
//                }
//            }
...
/* else{
if($deletedQuestionHelper->isQuestionValid("F2.3")) {
if($input['pan_promoter_file_check'] == 0) {
$fieldsArr['prom_pancard_file_path'] = $request->file('prom_pancard_file_path');
$rulesArr['prom_pancard_file_path'] = 'required';
$messagesArr['prom_pancard_file_path.required'] = 'PAN Card of Promoter File is required.';
}
}
}*/
...
if($deletedQuestionHelper->isQuestionValid("F2.3")) {
if($input['pan_promoter_file_check'] == 0) {
$fieldsArr['prom_idproof_file_path'] = $request->file('prom_idproof_file_path');
$rulesArr['prom_idproof_file_path'] = 'required';
$messagesArr['prom_idproof_file_path.required'] = 'Identity Proof of Promoter is required.';
}
}
}*/
  ...
  foreach ($input['equipementPurchase'] as $key => $file) {
    if ($m <= $input['num_equi_purchase']) {
      if (isset($file)) {
        ...
        //                            if($deletedQuestionHelper->isQuestionValid("F3.3")) {
        //                                if($input['equipementcopy_file_check_'.$m] == 0) {
        //                                    $fieldsArr['equipementPurchasefile'] = $file;
        //                                    $rulesArr['equipementPurchasefile'] = 'required';
        //                                    $messagesArr['equipementPurchasefile.required'] = 'Invoice Copy of Equipment Purchase ' . $m . ' File is required.';
        //                                }
        //                            }
      }
    } else {
  ...
  foreach ($input['invoiceBillFile'] as $key => $file) {
    if ($m <= $input['num_invoice_detail']) {
      if (isset($file)) {
        ...
        //                            if($deletedQuestionHelper->isQuestionValid("F3.4")) {
        //                                if($input['invoicecopy_file_check_'.$m] == 0) {
        //                                    $fieldsArr['invoicebillfile'] = $file;
        //                                    $rulesArr['invoicebillfile'] = 'required';
        //                                    $messagesArr['invoicebillfile.required'] = 'Copy of Invoice/Bill details ' . $m . ' file is required';
        //                                }
        //                            }
      }
    } else {
    ...
    if ($cnt <= $maxSecurityDocumentCount) {
      if (isset($file)) {
        ...
        if ($deletedQuestionHelper->isQuestionValid("F4")) {
          /*if($input['last_pro_val_report_' . $cnt] == 0) {
          $fieldsArr[$key] = $file;
          $rulesArr[$key] = 'required';
          $messagesArr[$key.'.required'] = 'Last Valuation-' . $cnt . '  File is required';
        }*/
      }
    }
  } else {
  ...
  foreach ($input['security_titlesearch_file'] as $key => $file) {
    ...
    if ($cnt <= $maxSecurityDocumentCount) {
      if (isset($file)) {
        ...
        if ($deletedQuestionHelper->isQuestionValid("F4")) {
          /*if ($input['pro_title_search_report_' . $cnt] == 0) {
          $fieldsArr[$key] = $file;
          $rulesArr[$key] = 'required';
          $messagesArr[$key.'.required'] = 'Property Title Search Report-' . $cnt . '  File is required';
        }*/
      }
    }
  } else {
  ...
  foreach ($input['security_propertytax_file'] as $key => $file) {
    ...
    if ($cnt <= $maxSecurityDocumentCount) {
      if (isset($file)) {
        ...
        if ($deletedQuestionHelper->isQuestionValid("F4")) {
          /* if ($input['pro_tax_card_' . $cnt] == 0) {
          $fieldsArr[$key] = $file;
          $rulesArr[$key] = 'required';
          $messagesArr[$key.'.required'] = 'Proper Tax Card copy-' . $cnt . '  File is required';
        }*/
      }
    }
  } else {
    ...
    if ($cnt <= $maxSecurityDocumentCount) {
      if (isset($file)) {
        ...
        if ($deletedQuestionHelper->isQuestionValid("F4")) {
          ...
          $fieldsArr[$key] = $file;
          $rulesArr[$key] = 'required';
          $messagesArr[$key.'.required'] = 'Occupation Certificate copy-' . $cnt . '  File is required';
        }*/
      }
    }
  } else {
  ...
  foreach ($input['security_societyshare_file'] as $key => $file) {
    ...
    if ($cnt <= $maxSecurityDocumentCount) {
      if (isset($file)) {
        ...
        if ($deletedQuestionHelper->isQuestionValid("F4")) {
          /*if ($input['society_share_cert_' . $cnt] == 0) {
          $fieldsArr[$key] = $file;
          $rulesArr[$key] = 'required';
          $messagesArr[$key.'.required'] = 'Society Share Certificate copy-' . $cnt . '  File is required';
        }*/
      }
    }
  } else {
  ...
  foreach ($input['security_712extract_file'] as $key => $file) {
    ...
    if ($cnt <= $maxSecurityDocumentCount) {
      if (isset($file)) {
        ...
        if ($deletedQuestionHelper->isQuestionValid("F4")) {
          /* if ($input['copy_7_12_extract_' . $cnt] == 0) {
          $fieldsArr[$key] = $file;
          $rulesArr[$key] = 'required';
          $messagesArr[$key.'.required'] = 'Copy of 7 - 12 Extract-' . $cnt . '  File is required';
        }*/
      }
    }
  } else {
  ...
  foreach ($input['security_lastsaledeed_file'] as $key => $file) {
    ...
    if ($cnt <= $maxSecurityDocumentCount) {
      if (isset($file)) {
        ...
        if ($deletedQuestionHelper->isQuestionValid("F4")) {
          /*if ($input['copy_last_sales_pur_' . $cnt] == 0) {
          $fieldsArr[$key] = $file;
          $rulesArr[$key] = 'required';
          $messagesArr[$key.'.required'] = 'Copy of Last Sales/Purchase Deed-' . $cnt . '  File is required';
        }*/
      }
    }
  } else {
  ...
  foreach ($input['muncipal_plan'] as $key => $file) {
    ...
    if ($cnt <= $maxSecurityDocumentCount) {
      if (isset($file)) {
        ...
        if ($deletedQuestionHelper->isQuestionValid("F4")) {
          /* if ($input['municipal_plan_count_' . $cnt] == 0) {
          $fieldsArr[$key] = $file;
          $rulesArr[$key] = 'required';
          $messagesArr[$key.'.required'] = 'Municipal Plan-' . $cnt . '  File is required';
        }*/
      }
    }
  } else {
  ...
  foreach ($input['electricity_bill'] as $key => $file) {
    ...
    if ($cnt <= $maxSecurityDocumentCount) {
      if (isset($file)) {
        ...
        if ($deletedQuestionHelper->isQuestionValid("F4")) {
        if ($input['electricity_bill_count_' . $cnt] == 0) {
        $fieldsArr[$key] = $file;
        $rulesArr[$key] = 'required';
        $messagesArr[$key.'.required'] = 'Electricity Bill of Security-' . $cnt . '  File is required';
      }
    }
  }*/
} else {
...
//        $redirectUrl = $this->generateRedirectURL('loans/terms-conditions', $endUseList, $loanType, $amount, $loanTenure, $loanId);
//        return Redirect::to($redirectUrl);
$redirectUrl = 'home';
return Redirect::to($redirectUrl);
}
...
public function getApprover($loanType, $endUseList = null, $amount = null, $loanTenure = null, $loanId = null)
{
 ...
 return view('loans.createedit', compact('subViewType', 'formaction','userProfileFirm', 'loan', 'loanId', 'loanType', 'endUseList', 'amount', 'loanTenure', 'validLoanHelper', 'bankApprovalStatus_1', 'bankApprovalStatus_2'));
  ...
  public function getRepayment($loanType, $endUseList = null, $amount = null, $loanTenure = null, $loanId = null)
  {
        ...
        foreach ($model as $key => $val) {
          if ($key == 'finyear_file' . $i . '_path') {
            ...
            if (isset($val)) {
             ...
             // $gurav = 'http://santoshpawar.com/images/santosh.jpg';
              $gurav = $fileHelper->getFileDownloadURL($val);
            }
          } else if ($key == 'pancard_file_path') {
            if (isset($val)) {
              $pancard_file = $fileHelper->getFileDownloadURL($val);
            ...
            if (isset($val)) {
              $shopestablish_file_path = $fileHelper->getFileDownloadURL($val);
            }
          } else if ($key == 'addproof_file_path') {
            if (isset($val)) {
              $addressproof_company_file = $fileHelper->getFileDownloadURL($val);
