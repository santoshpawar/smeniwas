    ...
    <div class="content">
      <div class="card">
        <div class="card-header card-chart" data-background-color="green">
          <h2 class="text-center"><a href="/loans">Apply New Loan</a></h2>
        </div>
      </div>
      <section>
          ...
          <table id="smeGrid" class="display responsive nowrap" cellspacing="0" width="100%">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><a href=" {{URL::to('loans/company-background/'. $loan->type. '/' .$loan->end_use. '/' .$loan->loan_amount. '/' .$loan->loan_tenure. '/'.$loan->companySharePledged .'/'.$loan->bscNscCode.'/'. $loan->id)}}">{!! isset($loan->getUserProfile->name_of_firm) ? $loan->getUserProfile->name_of_firm : '' !!} [# {!! $loan->id !!}]</a></td>
                @else
                ...
                <td class="{!! $statusValue !!}">
                  @if(isset($queryFromSMENiwas['niwas_query_status']) && $queryFromSMENiwas['niwas_query_status'] == 'Y' && isset($queryFromBank['bank_query_status'])
                  && $queryFromBank['bank_query_status'] == 'Y' && $user->isSME() && isset($queryValue->id) && $queryValue->id
                  != '')
                  <a href="{{URL::to(" messaging/ ".$queryValue->id)}}" title="View Query Details">Query Received From Bank & SMENiwas</a>                  @elseif(isset($queryFromSMENiwas['niwas_query_status']) && $queryFromSMENiwas['niwas_query_status'] ==
                  'Y' && $user->isSME() && isset($queryValue->id) && $queryValue->id != '')
                  <a href="{{URL::to(" messaging/ ".$queryValue->id)}}" title="View Query Details">SMENiwas Query</a> @elseif(isset($queryFromBank['bank_query_status'])
                  && $queryFromBank['bank_query_status'] == 'Y' && $user->isSME() && isset($queryValue->id) && $queryValue->id
                  != '')
                  <a href="{{URL::to(" messaging/ ".$queryValue->id)}}" title="View Query Details">Query Received From Bank</a>                  @else @if($statusValue == 'Rejected')
                  <a class="tooltips" href="#">{!! $statusValue !!}<span>{!! $queryFromSMENiwas->remark !!}</span></a> @else
                  {!! $statusValue !!} @endif @endif
                </td>
                <td>
                ...
                <td></td>
                @endif @if($user->isBankUser() || $user->isAnalyst())
