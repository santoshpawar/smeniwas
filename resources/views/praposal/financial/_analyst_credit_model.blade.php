...
{!! Form::hidden('model_type', Config::get('constants.CONST_ANALYST_MODEL_TYPE_CREDIT'))!!}
{!! Form::hidden('status', 1)!!}
<div class="card">
 <div class="card-header" data-background-color="green">
   <h4 class="title">Credit Model <span class="pull-right">{{ $userProfile->name_of_firm }}</span></h4>
        ...
        <div class="tab-content tab-design" style="padding-top:20px;padding-right: 5px;padding-left: 5px;">
            ...
            <div id="credit_model" style="padding: 5px;">
                ...
                <table class="table table-bordered">
                 ...
                 <div class="row">
                    ...
                    <tr style="background-color:#B8CCE4;">
                        <td>
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                <span><B>{{$creditCategory->label}}</B></span>
                            </div>
                        </td>
                        <td>
                            <div class="col-border col-md-2 col-sm-2 col-xs-2">
                                <B><span id = "category_weight_label_top_{{$categoryCounter}}" class = "control-label" style = "width: 100%;">{{$creditCategory->weight}}</span></B>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                <div class="row">
                                    {!! Form::hidden('analyst_model_rating_details['.$dimensionCounter.'][id]', $creditDimension->selected_rating_details_id) !!}
                                    {!! Form::hidden('analyst_model_rating_details['.$dimensionCounter.'][dimension_id]', $creditDimension->id) !!}
                                    ...
                                    <div class="col-md-12 col-sm-6 col-xs-3">
                                        {!! Form::label('points', ($creditCategory->weight * ($creditDimension->weight / 100)), ['class'=>'control-label']) !!}
                                    </div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td style="width: 13%;">
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                {!! Form::select('analyst_model_rating_details['.$dimensionCounter.'][is_applicable_label]', $yesNoOptions, $creditDimension->is_applicable, ['id' => 'applicable_'.$dimensionCounter, 'class' => 'form-control', 'style' => 'width: 100%;']) !!}
                                {!! Form::hidden('analyst_model_rating_details['.$dimensionCounter.'][is_applicable]', $creditDimension->is_applicable) !!}
                            </div>
                        </td>
                        <td style="width: 22%;">
                            <div class="col-md-12 col-sm-12 col-xs-12">
                               @if($dimensionCounter=='23')
                               {!! Form::select('analyst_model_rating_details['.$dimensionCounter.'][measure_id_label]', $creditDimension->measures->lists('label', 'id')->all(), $creditDimension->selected_measure_id, ['id' => 'measureValue_'.$dimensionCounter, 'class' => 'form-control', 'style' => 'width: 100%;']) !!}
                               @else
                               {!! Form::select('analyst_model_rating_details['.$dimensionCounter.'][measure_id_label]', $creditDimension->measures->lists('label', 'id')->all(), $creditDimension->selected_measure_id, ['id' => 'measureValue_'.$dimensionCounter, 'class' => 'form-control', 'style' => 'width: 100%;']) !!}
                               @endif
                               {!! Form::hidden('analyst_model_rating_details['.$dimensionCounter.'][measure_id]', $creditDimension->selected_measure_id) !!}
                           </div>
                       </td>
                       @else
                       <td>
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            {!! Form::select('analyst_model_rating_details['.$dimensionCounter.'][is_applicable]', $yesNoOptions, $creditDimension->is_applicable, ['id' => 'applicable_'.$dimensionCounter, 'class' => 'form-control', 'style' => 'width: 100%;',$setDisable]) !!}
                        </div>
                    </td>
                    <td>
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            {!! Form::select('analyst_model_rating_details['.$dimensionCounter.'][measure_id]', $creditDimension->measures->lists('label', 'id')->all(), $creditDimension->selected_measure_id, ['id' => 'measureValue_'.$dimensionCounter, 'class' => 'form-control', 'style' => 'width: 100%;',$setDisable]) !!}
                        </div>
                    </td>
                    @endif
                    <td>
                ...
                <tr style="background-color:#B8CCE4;">
                    <td>
                        <div class="col-md-12 col-sm-6 col-xs-12">
                            {!! Form::label('category_label','Total Score - ' . $creditCategory->label, ['class'=>'control-label']) !!}
                        </div>
                    </td>
                    <td>
...
Dimension.prototype.recalculate = function(measureId) {
    ...
    CreditModel.prototype.recalculateRating = function(){
        ...
        for(var categoryIndex=0; categoryIndex < this.categoriesList.length; categoryIndex++){
            ...
            if(grandTotalCalculatedPoints >= {{ $creditRatingPointsList->get("AAA+",90) }}){
                rating = "AAA+";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("AAA",87)}}){
                rating = "AAA";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("AAA-",85)}}){
                rating = "AAA-";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("AA+",82)}}){
                rating = "AA+";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("AA",79)}}){
                rating = "AA";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("AA-",75)}}){
                rating = "AA-";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("A+",72)}}){
                rating = "A+";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("A",69)}}){
                rating = "A";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("A-",65)}}){
                rating = "A-";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("BBB+",62)}}){
                rating = "BBB+";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("BBB",59)}}){
                rating = "BBB";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("BBB-",55)}}){
                rating = "BBB-";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("BB+",52)}}){
                rating = "BB+";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("BB",49)}}){
                rating = "BB";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("BB-",45)}}){
                rating = "BB-";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("B+",42)}}){
                rating = "B+";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("B",39)}}){
                rating = "B";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("B-",35)}}){
                rating = "B-";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("C+",32)}}){
                rating = "C+";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("C",29)}}){
                rating = "C";
            }else if(grandTotalCalculatedPoints >= {{$creditRatingPointsList->get("C-",25)}}){
                rating = "C-";
            }else{
                rating = "D";
            }
        ...
        $( document ).ready(function() {
            ...
            @foreach($analystModelsCategoriesList as $creditCategory)
            var category{{$categoryCounter}} = new Category({{ $categoryCounter }}, {{ $creditCategory->id }}, "{{ (isset($creditCategory->type) && strlen($creditCategory->type) > 0)? $creditCategory->type:null }}", "{!! $creditCategory->label !!}", {{ $creditCategory->weight }});
            @foreach($creditCategory->dimensions as $creditDimension)
