  ...
  <div class="sidebar" data-color="purple" data-image="{{ asset('/images/sidebar-1.jpg') }}">
   ...
   <div class="sidebar-wrapper">
    <div id="tab" class="btn-group leftside_tab" role="group">
      ...
      @endif
      <ul class="nav">
        @if(Auth::user()->isAnalyst() || Auth::user()->isBankUser() || Auth::user()->isAdmin() || Auth::user()->isApproveUser())
        <li>
       ...
       <li> <a class="btn btn-large btn-success btn-space lefttabbtn {{{ @$sub_view_type == "loans.financial._analyst_credit_model" ? 'active' : ''}}} {{{$validLoanHelper->CurrentTabStatus($loanId,'credit_model')}}}" href="{{{ URL::to('/loans/liquidity-model/' . $loanId)}}}" role="button"><i class="material-icons">payment</i>Share Pledge Model</a></li>
       @else
       <li> <a class="btn btn-large btn-success btn-space lefttabbtn {{{ !$sub_view_type == "loans.financial._analyst_credit_model" ? 'active' : ''}}} {{{$validLoanHelper->CurrentTabStatus($loanId,'credit_model')}}}" href="{{{ URL::to('/loans/credit-model/' . $loanId)}}}" role="button"><i class="material-icons">payment</i>Credit Model</a></li>
       @endif

       <li>
         @if(@$loan->companySharePledged=='')
         <a class="btn btn-large btn-success btn-space lefttabbtn" href="{{{ URL::to('/loans/data-cash-flow-model/' . $loanId)}}}" role="button"><i class="material-icons">payment</i>Liquidity Test Model</a>
         @endif
       </li> 

        <li>
         @if(@$loan->companySharePledged=='')
         <a class="btn btn-large btn-success btn-space lefttabbtn" href="{{{ URL::to('/loans/cash-flow-model/' . $loanId)}}}" role="button"><i class="material-icons">payment</i>Liquidity Assessment</a>
         @endif
       </li> 

       <li>
         @if(@$loan->companySharePledged=='')
         <a class="btn btn-large btn-success btn-space lefttabbtn" href="{{{ URL::to('/loans/cash-flow-criteria/' . $loanId)}}}" role="button"><i class="material-icons">payment</i>Liquidity Score</a>
         @endif
       </li> 
       ...
       <li>
         ...
         <a class="btn btn-large btn-success btn-space lefttabbtn {{{ @$sub_view_type == "loans.financial._analyst_collateral_model" ? 'active' : ''}}} {{{$validLoanHelper->CurrentTabStatus($loanId,'collateral_model')}}}" href="{{{ URL::to('/loans/collateral-model/' . $loanId)}}}" role="button"><i class="material-icons">payment</i>Collateral Model</a>
         @endif
       </li>
       @endif
       ...
       <li>
         @if(Auth::user()->isAnalyst() || Auth::user()->isApproveUser() || Auth::user()->isBankUser() || Auth::user()->isAdmin())
       ...
       <li>
         @if(Auth::user()->isAnalyst() || Auth::user()->isApproveUser() || Auth::user()->isBankUser()  || Auth::user()->isAdmin())
         <a style="width:100%;" href="{{{URL::to('loans/praposal/keyloanterm/'.$loanType.'/'.$endUseList.'/'.$amount.'/'.$loanTenure.'/'.$loanId)}}}" class="btn btn-large btn-success btn-space lefttabbtn {{{$sub_view_type == "loans._bank_approval" ? 'active' : ''}}} " role="button"><i class="material-icons">account_balance</i>Proposal key terms</a>
         @endif
       </li>

       <li>
         @if(Auth::user()->isApproveUser() || Auth::user()->isAdmin())
