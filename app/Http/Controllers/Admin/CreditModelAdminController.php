...
class CreditModelAdminController extends BaseAdminController {
    ...
    public function getIndex(){
        $categories = AnalystModelCategory::all();
         
        $sideTab = 'analystData';
        return view('admin.creditmodel.index', compact('sideTab','categories'));
    }
    ...
    public function getDimensions($categoryId){
        $dimensions = AnalystModelDimension::where('category_id','=',$categoryId)->orderBy('sortorder', 'asc')->get();
        $sideTab = 'analystData';
        return view('admin.creditmodel.dimensions', compact('sideTab','categoryId','dimensions'));
    }
    ...
    public function getMeasures($dimensionId,$categoryId){
        $measures = AnalystModelMeasure::where('dimension_id','=',$dimensionId)->get();
        $sideTab = 'analystData';
        return view('admin.creditmodel.measures', compact('sideTab','dimensionId','categoryId','measures'));
    }

    public function getEditDimension($id)
    {
        $dimensionData = AnalystModelDimension::find($id);
        $parentDimensionsList = array(NULL => '') + AnalystModelDimension::where('status','=', 1)->get()->lists('label', 'id')->all();
        ...
        $formaction = 'Admin\CreditModelAdminController@postSave';
        $sideTab = 'analystData';
        return view('admin.creditmodel.createedit',compact('sideTab','dimensionData','formaction','attributeStatus', 'chosenStatus','attributeIsApplicable', 'chosenIsApplicable', 'categoryId', 'parentDimensionsList','ratioList','dimensionType', 'modelsList','removeMandatory'));
    }
    ...
    public function getCreateDimension($categoryId)
    {
        ...
        $formaction = 'Admin\CreditModelAdminController@postSave';
        $sideTab = 'analystData';
        return view('admin.creditmodel.createedit',compact('sideTab','dimensionData','formaction', 'attributeStatus', 'chosenStatus','attributeIsApplicable', 'chosenIsApplicable','categoryId', 'chosenParentDimension',  'parentDimensionsList','ratioList','dimensionType', 'modelsList','removeMandatory'));
    }
...
//        dd($input);
        ...
        $data = null;
        if(isset($id)) {
            $data = AnalystModelDimension::find($id);
        }
        $rules = array(
            'label' => 'required',
            'description' => '',
        ...
        AnalystModelDimension::updateOrCreate(array('id' => $id), $input);
        session()->flash('flash_message','Credit-Model Dimension Details were successfully saved!');

        $redirectPath = 'admin/creditmodel/dimensions/'.$categoryId;
    ...
    {
        $categoryData = AnalystModelCategory::find($id);
        $attributeStatus = MasterData::attributeStatus();
        ...
        $formaction = 'Admin\CreditModelAdminController@postEditCategory';
        $sideTab = 'analystData';
        return view('admin.creditmodel.createeditcategory',compact('sideTab','categoryData','formaction','attributeStatus', 'chosenStatus','removeMandatory'));
    }
    ...
    public function postEditCategory(Request $request)
    {
        ...
        if(isset($id)) {
            $data = AnalystModelCategory::find($id);
        }
        $rules = array(
            'label' => 'required',
            'description' => '',
...
//        dd($data);
    ...
    public function getEditMeasures($id,$dimensionId,$categoryId)
    {
        ...
        $formaction = 'Admin\CreditModelAdminController@postEditMeasure';
        $sideTab = 'analystData';
        return view('admin.creditmodel.createeditmeasure', compact('sideTab','formaction','dimensionId','categoryId','questionMeasureData','attributeStatus','chosenStatus','attributeOperand','chosenOperand','removeMandatory'));
    }
...
//        dd($input);
        ...
        if(isset($id)) {
            $data = AnalystModelMeasure::find($id);
        }
        $rules = array(
            'label' => 'required',
            'description' => '',
        ...
        $this->validate($request, $rules);
        $input = $request->all();
        $creditMeasure = AnalystModelMeasure::updateOrCreate(array('id' => $id), $input);
        if($input['operand'] == '')
        {
        ...
        $redirectPath = 'admin/creditmodel/measures/'.$input['dimension_id'].'/'.$input['categoryId'];
        return Redirect::to($redirectPath)->withInput();
    }

    public function getDeleteMeasures($id)
    {
        $creditMeasure = AnalystModelMeasure::find($id);
