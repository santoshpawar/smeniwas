...
class Thread extends Eloquent
{
     ...
     * @var array
     */
    protected $fillable = ['loan_id','subject','is_replied'];
     ...
     *
     * @return \App\Models\Messenger\Message
     */
    ...
    {
        return $this->hasMany('App\Models\Messenger\Participant');
    }
    ...
    {
        $users = $this->participants()->withTrashed()->lists('user_id')->all();
    ...
    public function participantsString($userId=null, $columns=['name'])
    {
        ...
        $userNames = $participantNames->lists('users.name')->all();
    ...
    private function createSelectString($columns)
    {
        ...
        switch ($dbDriver) {
            case 'pgsql':
            case 'sqlite':
                $columnString = implode(" || ' ' || user_profiles.", $columns);
                $selectString = "(user_profiles." . $columnString . ") as name";
                break;
            case 'sqlsrv':
                $columnString = implode(" + ' ' + user_profiles.", $columns);
                $selectString = "(user_profiles." . $columnString . ") as name";
            ...
            default:
                $columnString = implode(", ' ', user_profiles.", $columns);
                $selectString = "concat(user_profiles." . $columnString . ") as name";
        }
